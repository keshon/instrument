/* Сверка документации с китом. Обе стороны разом.
   ...
   Заодно проверяются значения data-атрибутов: словари закрыты (docs/about/design-principles.md),
...
/* Стол примера имеет собственные свойства — --c у образца цвета, --v у
   строки шкалы. Они не токены кита и никогда ими не станут: это способ
   показать сам материал, а не часть интерфейса. Но и выдумкой они не
   являются, поэтому источником считается и стилевой файл стола. */
const STAGE = P('../site/internal/render/assets/demo.css');
if (existsSync(STAGE)) {
  const stage = readFileSync(STAGE, 'utf8');
  for (const m of stage.matchAll(/(?<![\w-])(--[a-z][\w-]*)\s*:/g)) tokens.add(m[1]);
  /* Как и у кита, свойства-входы объявления не имеют: --c приходит из
     разметки примера, потому что показываемое значение и есть содержание. */
  for (const m of stage.matchAll(/var\(\s*(--[a-z][\w-]*)/g)) tokens.add(m[1]);
}
