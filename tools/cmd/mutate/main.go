...
var mutations = []mutation{
	...
	// The two below are the shapes the row scanner used to walk past. Neither
	...
	{"a lightness of the ramp lied", "docscheck", "docs/foundations/colors.md",
		"| `--n-8` | 0.500 |",
		"| `--n-8` | 0.520 |",
		"the ramp tables live on the colour page, and only the reference was read"},
	{"the hue of a chart lied", "docscheck", "docs/foundations/tokens.md",
	...
	{"the side column names a page that is not there", "site", "site/internal/nav/nav.go",
		...
		"{\"blocks\", []string{\"console\", \"monitor\", \"chat\", \"dashboard\"}},",
	...
	{"a glyph that the sprite has no symbol for", "site", "site/internal/content/content.go",
		"p.Icon = \"i-p-\" + name",
		"p.Icon = \"i-p-\" + name + \"-x\"",
		"a <use> naming no symbol draws nothing at all: no error, no box, no mark — only somebody looking at the page"},
