...
type Theme struct {
	...
	// сочетается с любой из них: 4 акцента × 6 тем. В самом ките это
...
// то же на обоих концах, и один порог работает для всех шести тем.
...
// Step — модуль разницы светлоты между двумя поверхностями.
func Step(a, b RGBA) float64 { return math.Abs(Lightness(a) - Lightness(b)) }
