...
| `--focus-ring` | `--a-4` | `--a-3` | Кольцо фокуса — обычный акцент. От заливки, которую оно обводит, его отделяет зазор в 2px из `outline-offset`, а не цвет, подобранный ради одного этого случая |
...
| `--switch-track` | чёрный 45% | белый 36% | Переключатель обязан быть виден **пустым**, поэтому его дорожка несущая, 3:1, и кольцо, которое ей для этого требовалось, снято |
...
| Радиусы | `--radius-2xs` 2 · `--radius-xs` 4 · `--radius-sm` 6 · `--radius-md` 8 · `--radius-lg` 12 · `--radius-full`. У контролов — `--radius-control-sm/md/lg`, они едут за плотностью; у пометок (всё мельче 8px) — `--radius-mark`, он не едет |
