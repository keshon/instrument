...
api:
  ...
  - { name: "--radius-2xs", kind: "токен", value: "2px", doc: "Нижняя ступень лестницы. Пометки берут её через --radius-mark" }
  - { name: "--radius-mark", kind: "токен", doc: "Скругление пометки: всё, что кит рисует мельче 8px — точка, штрих, метр и дорожка" }
  - { name: "--radius-xs", kind: "токен", value: "4px", doc: "Инлайн: бейджи, чипы, клавиша" }
...
|---|---|---|
| `--radius-2xs` | 2px | Нижняя ступень. Пометки достают её через `--radius-mark`, а не по имени |
| `--radius-xs` | 4px | Инлайн: бейджи, чипы, клавиша |
...
Скругление ПОМЕТКИ берётся из `--radius-mark` по причине обратного свойства.
Мельче 8px лестница перестаёт предлагать выбор: когда два радиуса на одной
стороне в сумме больше самой стороны, браузер молча ужимает все радиусы бокса,
пока они не влезут. `--radius-xs` на точке 6px приезжает тройкой — это половина
бокса, то есть круг; на метре 6px — той же тройкой, то есть пилюлей. А там, где
вторая сторона бокса — это ДАННЫЕ (ширина отрезка дорожки, ширина штриха),
форма начинает меняться вместе с данными. Ярус пометок — единственная ступень,
которая переживает любую пометку в любом кегле.
