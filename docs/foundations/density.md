...
атрибут на панели перенастраивает всё внутри неё, потому что меняется ровно
ярус ролей, а компоненты собственных чисел не содержат.
...
|---|---|
| `compact` для плотного табличного вида: очередь, лог, инспектор | **`compact` для основной навигации** — цели нажатия уходят ниже порога, см. [предупреждение](#цели-нажатия) и [оболочку](../layout/shell.md) |
...
> **Предупреждение.** WCAG 2.5.8 (AA) требует цель нажатия не меньше 24×24
> CSS-пикселей. В плотности `compact` контрол размера `sm` даёт **22px** —
> ниже порога. Это осознанный размен: плотный табличный вид существует ради
> плотности.
