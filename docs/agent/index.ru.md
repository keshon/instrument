...
Пятнадцать компонентов, и все они про одно: **показать человеку работу машины,
...
СОСЕДЯХ, и повторённое пятнадцать раз оно расходится с ними по одной строке за
правку.
...
| Один шаг обмена, который держит всё перечисленное | [Реплику](./turn.md) | Строка очереди плоская, а шаг — один вызов: ответа не держит ни то ни другое |
| Черновик, который машина сделала перед ответом | [Размышление](./thinking.md) | Шаг это вызов с результатом, который может провалиться; здесь статуса нет вовсе |
| Поле, в котором пишут следующую реплику | [Поле ввода](./composer.md) | Поле рядом с панелью инструментов — два объекта, и фокус подсвечивает половину одного |
