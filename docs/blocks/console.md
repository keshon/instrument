...
source: src/kit.css
template: splash
title-en: "Run console"
group-en: "Blocks"
---

Экран, ради которого кит и затевался: слева список того, что машина делает,
справа — подробности выбранного. Ни одного класса, кроме китовых, и ни одной
строки скрипта.
...
| Список | [Строка задачи](../agent/task.md) в `role="listbox"`: клавиатура от [kit.js](../foundations/behavior.md) |
...
наверху нет намеренно: сводка за сутки — это отдельная задача и отдельный экран,
...
**Полосы «Отмена / Сохранить» здесь нет намеренно.** Она обещает, что до
нажатия ничего не произошло, — и почти всегда врёт: свитч уже сработал, поле уже
отправило запрос. Честнее применять сразу и сказать об этом одной строкой.
