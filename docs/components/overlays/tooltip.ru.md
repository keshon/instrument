...
api:
  ...
  - { name: "inst-tooltip-text--end", kind: "модификатор", doc: "Хвостовой край плашки по хвостовому краю триггера, а не по центру. Для последнего контрола в строке, где центрированная плашка уходит за контейнер" }
...
Плашка центрируется по триггеру, и на **последнем контроле в строке** это
уводит её за край: `nowrap`-плашка в несколько раз шире глифа, который она
поясняет, и обрезается ровно та половина, которую читают.
`inst-tooltip-text--end` ставит её хвостовой край по хвостовому краю триггера —
правый в LTR, левый в RTL, одним правилом. Что контрол последний, знает
вызывающий, а не кит.
