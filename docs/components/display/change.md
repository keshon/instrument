...
|---|---|
| Судьба файла в списке изменений или в дифе | **Состояние процесса** — [бейдж](./badge.md) с тоном: «идёт», «готово», «упало» |
| Значение из закрытого списка того, что случилось | **Оценка «хорошо / плохо»** — тональный словарь. «Удалён» не оценка |
| Метка в конце строки файла | **Количество строк** — `+18 −2` в [дифе](../../agent/diff.md) |
...
| Контраст | Разные пороги у разных носителей: контур меточный 3:1, знак текстовый 4.5:1. Проверяется обходом по пикселям в шести темах |
...
[Бейдж](./badge.md)
[Диф](../../agent/diff.md)
[Строка задачи](../../agent/task.md)
[Палитра графика](../charts/palette.md)
