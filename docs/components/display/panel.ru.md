...
api:
  ...
  - { name: "inst-panel--plain", kind: "модификатор", doc: "Без обводки: границу держит один перепад поверхности. Для крупных областей" }
...
<div class="inst-panel inst-panel--plain">
  <div class="inst-panel-header"><span class="inst-panel-title">Прогоны</span></div>
  <div class="inst-panel-body">Без обводки: границу держит перепад от страницы к листу.
    Только для крупных областей.</div>
</div>
```
...
|---|---|---|
| `--plain` только на крупной области | да | Границу тогда держит перепад 1.07 — замерено, что на 201 000 px² он держится, а на 36 000 теряется. Мелкой панели обводка положена |
