...
**Пять значений, а не «сколько понадобится».** Словарь перечислим, поэтому
шестой тон нельзя добавить случайно — только осознанно, вместе со строкой в
конституции.

**Информационного тона нет, и это решение.** Синий занят акцентом —
интеракцией и состоянием «идёт». Третья роль сделала бы его нечитаемым, а
второй синий кит запрещает. Информационное сообщение берёт `neutral`.

**У тона два передних плана, а не один.** `--tone-ink` для текста (порог
4.5:1) и `--tone-mark` для метки — точки, каретки, заливки меры (порог 3:1, но
ещё и против дорожки). У статусов они совпадают, у акцента расходятся, и
обязаны.
