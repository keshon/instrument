...
**Галочка — маска, а не символ в `content`.** Форму рисует маска, цвет
приходит токеном: цвета внутри data-URI кит запрещает, иначе тема не сможет
его сменить.

**Дорожка свитча несёт кольцо `--border-control`.** У свитча граница и есть
контрол, поэтому 3:1 с неё спрашивается — в отличие от дорожки меры, которая
ничего не ограничивает и имеет право быть тише.

**Метка и заливка кнопки — один и тот же шаг акцента.** Иначе на одном экране
соседствуют два разных синих: чекбокс одного оттенка, бегунок рядом другого.
