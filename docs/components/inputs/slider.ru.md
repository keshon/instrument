...
Клавиатура, роль `slider` и `aria-valuenow` — от платформы. Руками нужен только
`aria-valuetext` там, где число само по себе непонятно: «6 агентов» вместо «6».

Цель нажатия у бегунка 14px, **ниже 24px по WCAG 2.5.8**: нажатие принимает
дорожка на всю высоту `--size-icon`, а положение достижимо с клавиатуры. Там,
где слайдер — единственный способ задать значение, рядом ставится [числовое
поле](./num-field.md): точность не его работа.
