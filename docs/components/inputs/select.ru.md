...
Клавиатура целиком нативная: стрелки, `Home` и `End`, поиск по первым буквам,
`Enter`. Подпись даёт `<label for>` или `aria-label` — первый `<option>`
подписью не является.

Шеврон нарисован маской и получает `pointer-events: none`, чтобы клик по
стрелке открывал список, а не проваливался мимо; в `forced-colors` маска
теряет цвет фона и потому переводится на `CanvasText` явно.

Нативный список остаётся нативным намеренно: на телефоне он превращается в
системное колесо, и самодельный выпадающий это теряет.
