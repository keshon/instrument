...
api:
  ...
  - { name: "--fill", kind: "переменная", value: "0%", doc: "Доля заполненной дорожки. **Канал данных**: разметка объявляет начальное значение, `instrument.js` держит его при перетаскивании" }
...
Нативный `input[type=range]`, приведённый к метрике библиотеки. Значение выбирают
...
между цветами нет, поэтому запрет на градиенты это не нарушает: тот же
приём, что у заливки [спарклайна](../charts/sparkline.md).

Долю держит `--fill`. CSS не умеет прочитать `value` у `input`, поэтому её
считает [instrument.js](#js) — но **начальное значение объявляет разметка**, и без
...
:::note
**Число рядом с бегунком держит [instrument.js](#js).** Связать положение с текстом
...
Пример живой: потяните бегунок — число меняет `instrument.js`.
...
Пример в разделе «Вывод значения» живой: потяните бегунок, число рядом меняет `instrument.js`.
...
| | |
|---|---|
| Клавиатура | Стрелки — шаг, `PageUp` и `PageDown` — крупный шаг, `Home` и `End` — концы диапазона. Всё нативное, `instrument.js` ничего не перехватывает |
