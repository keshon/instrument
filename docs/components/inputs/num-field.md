...
«отсюда тянут», но само изменение значения тягой пишет приложение. Без скрипта
поле остаётся обычным числовым вводом, и он полностью работает: цифры,
стрелки, колесо, вставка.
:::
