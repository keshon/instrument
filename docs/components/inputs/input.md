...
**Поле стоит во врезе, а не на цвете панели.** Пока фон поля совпадал с фоном
панели, его единственной границей была рамка на контрасте 1.31 — то есть поля
на панели не было видно вообще.

**Подсказка и ошибка в одном слоте.** Ошибка заменяет подсказку, а не сдвигает
форму вниз. Форма не перекладывается в момент заполнения.

**Наведение уступает ошибке селектором, а не порядком строк.** Порядок
состояний — вопрос корректности, и надеяться на то, что правила стоят в нужной
последовательности, здесь нельзя.
