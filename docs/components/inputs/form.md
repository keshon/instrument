...
**Подпись отстоит от поля дальше, чем подсказка.** Подсказка принадлежит полю
теснее, и одинаковый зазор склеивал три разные вещи в одну.

**Подпись сбоку выравнивается по первой строке контрола, а не по его верху.**
Иначе она уезжает вверх у многострочного поля и у поля с приставкой.

**Полоса действий липкая, а не в конце потока.** В длинной форме кнопка
«Сохранить», до которой надо доскроллить, — это кнопка, которую не нажмут.
