...
**Липкой полоса становится только внутри `inst-panel-body`.** Липкая полоса
обязана быть непрозрачной над тем, по чему она едет, а единственная
поверхность, про которую это известно, — поверхность области. Снаружи полоса
приезжала с цветом страницы поверх белой панели и выглядела прорезью в ней:
кнопка «Сохранить» посреди чужих полей.

Практический вывод: **полоса живёт там же, где поля, которые она сохраняет.**
Форма на всю страницу, у которой действия вынесены наружу панели, — это не
длинная форма, а панель, из которой выпала кнопка.

Главное действие идёт **первым**. Порядок чтения слева направо
