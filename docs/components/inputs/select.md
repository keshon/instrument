...
**Шеврон рисуется маской на псевдоэлементе обёртки.** Только так его цвет
остаётся токеном, а положение — логическим: в RTL он уезжает вместе с текстом.
Прежние два `linear-gradient` давали сплошной треугольник, прибитый к
физическому правому краю.

**Нативный `<select>`, а не своя реализация.** Своя обошлась бы в бегущий
`tabindex`, поиск по буквам, скролл к выбранному, поведение на телефоне и
позиционирование — и всё равно проиграла бы системной по мелочам.
