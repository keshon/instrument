...
который никто не смотрел, — ни то ни другое: он берёт `data-empty`, то же слово,
каким это называет `inst-history-tick`.

То же утверждение, нарисованное намеренно по-другому. На полосе отсутствие
рисуется САМИМ отсутствием — метки нет вовсе, — и это работает, потому что у
штриха есть соседи: дырка между двумя цветными кирпичами читается дыркой. У
образца соседей нет, и тот, который не рисует ничего, — это не запись об
отсутствии, а пропавший образец рядом с осиротевшим словом. Поэтому в легенде
он берёт цвет, который у земли полосы в этот час: дорожку.
