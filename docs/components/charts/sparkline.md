...
`--chart-1` … `--chart-6` · `--control-h-md` · `--surface-raised` ·
`--size-marker`
...
| Контраст | Линия берёт категориальный тон и держит 3:1 против поверхности в четырёх темах. Заливка с альфой `0.12` — декоративная и на порог не проверяется: значение несёт линия |
