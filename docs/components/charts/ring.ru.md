...
api:
  ...
  - { name: "inst-ring--sm", kind: "модификатор", doc: "Бокс из `--control-h-sm`, обводка 2.5. В строке контролов" }
  - { name: "inst-ring--xs", kind: "модификатор", doc: "Бокс из `--icon-size` — там, где стоит глиф, а не контрол. Обводка 2, оптическая масса иконки" }
  - { name: "--ring-size", kind: "переменная", value: "--control-h-lg", doc: "Бокс. Задаётся модификаторами размера; вызывающий может задать напрямую" }
  - { name: "--ring-stroke", kind: "переменная", value: "3", doc: "Обводка, в единицах viewBox. Масштабируется вместе с боксом, поэтому снижается отдельно" }
...
|---|---|---|
| `role="img"` на `<svg>` | когда стоит отдельно | Иначе у графики нет роли, и `aria-label` на ней не гарантирован. **Внутри подписанного контрола** кольцо получает `aria-hidden="true"`, а имя несёт контрол: два имени на одном объекте читаются дважды |
...
| Размер | Бокс | Обводка | Где |
|---|---|---|---|
| по умолчанию | `--control-h-lg` | 3 | Рядом с текстом карточки. Единственный, внутрь которого влезает подпись |
| `inst-ring--sm` | `--control-h-sm` | 2.5 | В строке контролов, рядом с кнопкой |
| `inst-ring--xs` | `--icon-size` | 2 | Там, где стоит **глиф**: конец строки сессии, рядом с моделью и усилием |

Самая мелкая ступень — не высота контрола. Там кольцо не контрол среди
контролов, а ещё один глиф в строке, поэтому оно читает `--icon-size` — ту же
переменную, что читает каждая иконка в этой строке, — и двигается вместе со
строкой. С `--control-h-xs` выходило 22px против глифов в 14 и читалось как
третий объект между ними.

Обводка следует тому же доводу: иконка нарисована толщиной 1.5 на сетке 16,
то есть 9.4%, а сетка кольца — 20, значит 2 даёт ту же оптическую массу.

**Подпись вниз не идёт.** `--text-2xs` — нижняя граница шкалы кегля, и внутрь
22px она не влезает, тем более внутрь 14. Ниже `--control-h-lg` число уходит
рядом с кольцом или во [всплывающую подсказку](../overlays/tooltip.md): дуга,
рядом с которой нигде нет числа, читается только по длине и по цвету.

```html preview
<span class="inst-ring-wrap">
  <svg class="inst-ring inst-ring--sm" viewBox="0 0 20 20" role="img" aria-label="Израсходовано 40% контекста">
    <circle class="inst-ring-track" cx="10" cy="10" r="8"/>
    <circle class="inst-ring-fill" cx="10" cy="10" r="8" style="--value:0.4"/>
  </svg>
</span>
<span class="inst-tooltip">
  <span class="inst-ring-wrap" aria-describedby="tt-ring-xs-ru">
    <svg class="inst-ring inst-ring--xs" viewBox="0 0 20 20" role="img" aria-label="Израсходовано 90% контекста">
      <circle class="inst-ring-track" cx="10" cy="10" r="8"/>
      <circle class="inst-ring-fill" cx="10" cy="10" r="8" style="--value:0.9"/>
    </svg>
  </span>
  <span class="inst-tooltip-text" role="tooltip" id="tt-ring-xs-ru">Контекст израсходован на 90%</span>
</span>
```
