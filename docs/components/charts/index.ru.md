...
Семь компонентов, и ни один из них не график в обычном смысле: у них нет осей.
Это **показания**, встроенные в интерфейс, — доля, состав, тренд, расшифровка
цветов.
...
СОСЕДЯХ, и повторённое семь раз оно расходится с ними по одной строке за
правку.
...
| Из чего целое **состояло** — несколько исходов сразу | [Полосу состава](./share.md) | Мера сравнивает одну величину с потолком, здесь части сравниваются друг с другом |
...
| Сколько чего-то за день, по неделям | [Тепловую карту](./heatmap.md) | Спарклайн проводит линию сквозь время, тепловая карта даёт каждому дню свой квадрат |
...
| **Когда** что-то случилось, а не сколько его было | [историю проверок](../../agent/history.md) | Полоса состава не умеет в порядок: те же 8% — это один долгий провал или девяносто разбросанных |
| Исходы по дням, а не количество | [историю проверок](../../agent/history.md) | В тепловой карте нет провалов: её ось — интенсивность, а `ok`/`warn`/`error` — другой вопрос |
