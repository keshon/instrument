...
Seven components, and not one of them is a chart in the ordinary sense: they
...
statement about NEIGHBOURS, and repeated seven times it parts from them by one
line per edit.
...
| How much of something per day, across weeks | [The heatmap](./heatmap.md) | A sparkline draws a line through time; a heatmap gives every day a square of its own |
...
| Outcomes over days rather than a quantity | [a check history](../../agent/history.md) | A heatmap has no failures in it: its axis is intensity, and `ok`/`warn`/`error` is a different question |
