...
показывает, что текст ещё поступает. Обе — метки **без подписи**, и потому обе
берут `--tone-mark`, а не `--tone-ink`.
...
метки без подписи, порог 3:1, но ещё и против дорожки. У статусов они
совпадают, у акцента расходятся.
