...
needs-js: Закрытие баннера — слой приложения. Кит не даёт крестика намеренно
api:
...
Ничего, и это решение, а не пробел. Кит не рисует крестика: баннер сообщает о
состоянии системы, а состояние не исчезает оттого, что его закрыли. Закрытие
