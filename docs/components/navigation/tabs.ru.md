...
api:
  ...
  - { name: "--surface-recessed", kind: "токен", doc: "Дорожка, на которой лежит ряд чипов. Она и позволяет выбранному чипу быть приподнятым" }
  - { name: "--control-h-sm", kind: "токен", doc: "Высота чипа. Набор отступом давал ряд выше соседней кнопки" }
  - { name: "--size-icon-sm", kind: "токен", doc: "Значок, который несёт чип: метка файла, мельче значка контрола" }
  - { name: "--control-pad-sm", kind: "токен", doc: "Внутренний отступ чипа по горизонтали. Чип — контрол и берёт меру контрола" }
  - { name: "--control-h-xs", kind: "токен", doc: "Коробка крестика. Квадрат с полем вокруг глифа" }
  - { name: "--surface-raised", kind: "токен", doc: "Поверхность выбранного чипа, на два шага выше дорожки" }
...
Стрелки, `Home`, `End` и roving `tabindex` — работа
[`instrument.js`](../../foundations/behavior.md). Показ нужной панели —
приложения: кит не знает, какая панель за какой вкладкой.
...
Выполняет контракт роли `role="tablist"`: `←` и `→` между вкладками, `Home`,
`End`, обход по кругу, один `Tab` на всю полосу. `aria-selected` переносится и
по стрелке, и по клику.
...
Ряд — **утопленная дорожка**, а выбранный чип на ней **приподнят**. Одной
приподнятости мало, когда ряд лежит на приподнятой поверхности: разница в один
шаг светлоты — почти ничто. Поэтому вниз уходит ряд, а не вверх чип, и между
ними получается два шага. Глубину несёт порядок светлоты — как и везде в ките.
