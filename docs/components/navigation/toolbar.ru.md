...
api:
  ...
  - { name: "inst-toolbar-label", kind: "класс", doc: "Имя группы контролов. Тише того, что называет, и контрол должен указывать на него через `aria-labelledby`" }
...
| `inst-toolbar-label` | Называет группу, которая идёт за ним | Когда по самим контролам не видно, в чём вопрос |
...
Подпись — наоборот, её читают. Сегментированный переключатель из четырёх слов
показывает варианты, но не вопрос, и полоса, которая с него начинается,
нечитаема, пока читатель не догадается, что именно выбирают. Подпись несёт не
только слово, но и связь — контрол, который она называет, указывает на неё:

```html
<span class="inst-toolbar-label" id="tb-act">Действие</span>
