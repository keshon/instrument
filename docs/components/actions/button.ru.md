...
Вес отвечает на «насколько громко кнопка просит нажать себя», и ответ
измерим: **присутствие** — это площадь, взвешенная шагом светлоты к земле.
...
Монотонна во всех пяти темах, соседние ступени расходятся не меньше чем на
0.022 — это проверяет `cmd/contrast` парой «лестница: soft против default».
Рамка у outline **декоративная**, а не несущая.
...
Колонкой становится **текст**, поэтому `.inst-btn-stack` обязателен: у такого
контрола есть ведущая иконка и шеврон, и колонка из всей кнопки положила бы их
сверху и снизу. Две строки — свойство этого варианта, а не разрешение переносить
любую кнопку.
...
Подпись не убирается из потока, а гасится цветом: ширина держится, и скринридер
её по-прежнему читает. Фокус кнопка сохраняет — `disabled` выбросил бы её из
обхода прямо под руками у того, кто пришёл с клавиатуры, — а снимается только
мышь: защита от второго нажатия принадлежит обработчику.
...
поэтому квадрат держится во всех трёх размерах и при любой плотности.
...
Несколько **разных действий**, поставленных вплотную, — чем группа отличается
от сегментации и от чипа, сказано в [индексе раздела](./index.md).
