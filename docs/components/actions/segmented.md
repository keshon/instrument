...
Состояние приходит из разметки: библиотека его рисует, но не вычисляет. Без ролей `aria-checked`
...
| Выбранное значение всегда ровно одно | **Несколько включённых сразу** — [чип](./chip.md): у сегментированного выбранный один по устройству |
| Выбор виден целиком и переключается часто | **Переключение вкл/выкл** — свитч: у одного варианта нет пары |
| Варианты называются одним-двумя словами | **Переход между разделами** — [вкладки](../navigation/tabs.md): это роутинг, а не выбор значения |
...
Отличать от [вкладок](../navigation/tabs.md) просто: вкладки меняют **раздел**
(другое содержимое страницы), сегментированный — **параметр** того же
содержимого. Одиннадцать мониторов остаются теми же одиннадцатью, меняется
только окно, за которое их показывают.
...
| Не только цвет | Выбранный несёт **два** признака, и ни один не цвет: поднятую поверхность и кольцо рамки |
