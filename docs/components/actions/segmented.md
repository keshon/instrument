...
**Выбранный получает кольцо рамки, а не тень.** `box-shadow` здесь очерчивает
границу, а не поднимает элемент над плоскостью: тень в этом ките означает
«временное и сверху», и её имеют право носить только поповер и модалка.

**Роль `radiogroup`, а не `tablist`.** Раньше стояла вторая — без единой
панели, то есть роль была невалидной, и на неё были завязаны стили. Вкладка
обещает скринридеру панель, которую откроет; сегментированный контрол ничего
не открывает, он меняет значение.
