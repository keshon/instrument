...
| | |
|---|---|
| Клавиатура | `Tab` — фокус, `Enter` и `Space` — нажатие. Всё от нативного `<button>`, `instrument.js` ничего не перехватывает |
...
| Контраст | Подпись каждого варианта проверена на 4.5:1 в пяти темах. Заливка `primary` ограничена сверху по светлоте: светлее `0.56` белая подпись перестаёт держать порог |
