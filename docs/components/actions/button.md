...
`primary` ограничена сверху по светлоте: светлее `0.56` белая подпись
перестаёт держать порог. Цель нажатия — 32px у `md` и 26px у `sm`; в
...
:::dont Настоящий `<input>` ради вида
Подменять кнопку полем — ложь скринридеру про то, что здесь можно печатать, и
сломанная клавиатура: поле не открывается пробелом.
:::
