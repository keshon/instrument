...
Ещё две строки держатся независимо от того, стоит ли в них что-нибудь:
[строка меню](./menubar.md) над всем и [строка состояния](./statusbar.md) под
всем, обе во всю ширину — они принадлежат окну, а не рабочей области. Пустая
строка занимает ноль пикселей, а зазора между строками шелл не ставит, поэтому
держать их бесплатно — и это избавляет приложение от собственной сетки вокруг
шелла. Ставьте любую из них ребёнком шелла; порядок в разметке не важен.
...
Рабочая область объявлена `@container`-областью под именем `app`: её ширина
приходит от сетки оболочки, а не от содержимого, поэтому объявлять её
контейнером безопасно.

```css
/* Своё правило под узкую рабочую область */
