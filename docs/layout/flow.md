...
прямоугольником. Поля и карточки ширину колонки, наоборот,
занимают. Список закрытый и совпадает с инлайновыми компонентами библиотеки.
...
column`, написанный руками, растянет их заново. Это довод взять примитив, а
не писать колонку.
...
Утилит отступов в библиотеке **нет и не будет**, и это не эстетика.

Шкала разрежена сверху: между соседними шагами разрыв растёт, чтобы
...
библиотеки в разметку каждого экрана. Через полгода одинаковые на вид экраны
...
рассуждение — в [принципах дизайна](../about/design-principles.md).

:::do Зазор ставит контейнер
Нужен воздух между блоками — положите их в стопку. Шаг выбирается модификатором.
:::

:::dont Отступ у элемента
Он складывается с внутренним отступом контейнера по вертикали и не складывается
по горизонтали. Блок отъезжает от рамки неравномерно.
:::

:::do Три шага, названные намерением
`--tight`, умолчание, `--loose`. Плотность перенастраивает все три разом.
:::

:::dont «Чуть побольше» числом
Через полгода одинаковые на вид экраны расходятся на два пикселя в тридцати
местах, и чинится это только поштучно.
:::
...
[Ритм и форма](../foundations/spacing.md)
