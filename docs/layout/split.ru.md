...
| `inst-stack` внутри половины, если блоков больше одного | да | Половина сплита — это доля ширины, а не колонка потока: своего зазора у неё нет, и две панели встают вплотную. Зазор ставит [стек](./flow.md), сама половина его ставить не может — класс вешают и на компонент (`inst-split-side inst-card`), и тогда зазор влез бы внутрь карточки |
