...
|---|---|
| Постоянные показания окна: ветка, кодировка, режим, счётчик ошибок | **Ряд действий над содержимым** — [тулбар](../components/navigation/toolbar.md): он держит контролы и потому вдвое выше |
| Значение, которое меняется само и его читают краем глаза | **Сообщение уровня экрана** — [баннер](../components/feedback/banner.md) |
| Место дороже подписи | **Итог таблицы** — `tfoot`, он стоит под своими данными |
...
| Контраст | `--text-secondary` на `--surface-sunken`, выше 4.5:1 в шести темах. Кегль `--text-2xs` — половина минимального кегля, ниже него размера нет |
